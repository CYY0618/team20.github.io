...
<head>
    ...
    <script>
      function add() {
            var signUp = parseInt(document.forms["member_signup"].elements["sign_nec"].value);
            //存取id為area1的表單元素值，且轉換成整數型別
            if (signUp == "" || isNaN(signUp)){
                alert("為必填欄位並輸入數值")
                return;
            }
      }
      ...
      <main>
        <section>
          <div class="sign">
            ...
            <form method="post" action="signup_add.jsp" id="member_signup">
              姓名Name:<br>
              <input type="text" name="name" size="20" id="sign_nec" required><br>
              電話Phone:<br>
              <input type="text" name="phone" size="20" id="sign_nec" required><br>
              地址Address:<br>
              <input type="text" name="address" size="20"><br>
              信箱Email:<br>
              <input type="text" name="email" size="20" id="sign_nec" required><br>
              密碼Password:<br>
              <input type="text" name="password" size="20" id="sign_nec" required><br>
